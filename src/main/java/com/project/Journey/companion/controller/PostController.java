    package com.project.Journey.companion.controller;
    ...
    public class PostController {
        ...
        @Operation(summary = "게시글 수정", description = """
            ...
            ✅ 수정 가능한 필드:
            - title, content, destination, startDate, endDate, max_participants, country
            - coverImageUrl, imageUrls
        
            ✅ 사용 방법:
            - `post` : JSON 형식의 게시글 정보 (`PostUpdateRequestDTO`)
            - `newCoverImage` : 커버 이미지를 새로 등록하고 싶을 경우 (기존 이미지 삭제 후 교체)
            - `newImages` : 첨부 이미지를 새로 추가하고 싶을 경우 (기존 이미지 유지하려면 imageUrls에 포함시켜야 함)
        
            ✅ 이미지 제거 방법:
            - coverImageUrl을 빈 문자열("")로 주면 커버 이미지가 삭제됩니다.
            - imageUrls에 없는 이미지는 모두 삭제됩니다 (부분 유지하려면 명시적으로 포함 필요).
            ...
            📌 예시 JSON (post 필드):
            {
              ...
              "coverImageUrl": "",
              "imageUrls": [
                "https://s3.bucket.com/image1.jpg"
              ]
            }
        ...
        public ResponseEntity<PostResponseDTO> updatePost(
            ...
            try {
                ...
                PostUpdateRequestDTO dto = mapper.readValue(postJson, PostUpdateRequestDTO.class);
                PostResponseDTO response = postService.updatePostById(post_id, dto, newImages, newCoverImage);
